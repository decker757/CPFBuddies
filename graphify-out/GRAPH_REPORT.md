...
## Corpus Check
- 88 files · ~33,092 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 808 nodes · 1758 edges · 49 communities (36 shown, 13 thin omitted)
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 399 edges (avg confidence: 0.61)
...
Cohesion: 0.09
Nodes (42): AgentRole, StrEnum, Stable codes. The dashboard and the audit log render these directly., ReasonCode, True when `signature` over `digest` was produced by `expected_address`., signed_by(), check_basket_binding(), check_charge_targets_mandate() (+34 more)

### Community 8 - "test_store_contract.py"
Cohesion: 0.13
Nodes (33): FixtureRequest, _dynamo_stores(), _entry(), held_verdict(), _hold(), Any, datetime, One suite, run against both store implementations.  The in-memory stores are not (+25 more)

### Community 9 - "VerifierService"
Cohesion: 0.12
Nodes (24): Decision, Decides whether a charge may settle against a mandate., VerifierService, Boundary cases, where a check either holds exactly or does not hold at all.  The, Thresholds are config. This is the test that proves it., A silent policy change would make two verdicts incomparable., Addresses are normalised at parse time, so casing is never a mismatch., The basket may match while the price has been raised underneath it. (+16 more)
...
Cohesion: 0.11
Nodes (30): IllegalBinding, A `bind` tried to do something other than fill in an empty field.      Binding m, MandateService, Issues and manages the credentials that authorise spending., The address the Verifier must be configured to trust., _mint(), The Mandate Service: what it will and will not let you do.  The interesting test, The Verifier's replay check reads this index. (+22 more)
...
Cohesion: 0.12
Nodes (11): MandateStatus, StrEnum, Timestamp, A copy at the next lifecycle step. Records are never mutated., Lifecycle of a mandate. Only MINTED and BOUND can still spend., MandateStore, Persistence for mandates, with the conditional writes that make     one-time con, Persist a freshly minted mandate and claim its nonce.          Raises `MandateAl (+3 more)

### Community 15 - "FakeKmsClient"
Cohesion: 0.08
Nodes (31): PublicKey, address_from_public_key(), KmsSigner, _normalise_s(), _pack(), _parse_der_signature(), Any, A signer whose private key lives in KMS and never leaves it.  KMS will happily s (+23 more)
...
Cohesion: 0.14
Nodes (14): ClientError, DynamoReviewHoldStore, _explain_failed_creation(), _failed(), _mandate_item(), _nonce_item(), Any, datetime (+6 more)

### Community 18 - "LocalSigner"
Cohesion: 0.09
Nodes (17): address_of(), secp256k1 primitives, in the shape Avalanche expects.  A signature on the wire i, Sign a 32-byte digest, returning `0x` + r || s || v with v in {27, 28}., The lowercase address that `sign_digest` will produce signatures for., Recover the signing address, or None if the signature is unusable.      Returnin, recover_address(), sign_digest(), LocalSigner (+9 more)

### Community 19 - "Mandate"
Cohesion: 0.15
Nodes (13): Self, The strictest decision in the list; PASS when the list is empty., _named_check(), The invariants that make the verdict model mean what we say it means.  Two claim, No judgement check runs once a fact has already settled it., The trace is a record of what was consulted, not of what exists., The one place the 'facts cannot be overridden' rule is enforced.      If a futur, test_a_deterministic_check_asking_for_review_still_fails() (+5 more)

### Community 20 - "mandate_digest"
Cohesion: 0.06
Nodes (43): An honestly hashed mandate carrying somebody else's signature.          This is, timedelta, Issue a mandate for a budget and an intent.          No merchant and no basket:, Mandate, BaseModel, The mandate: a signed credential scoped to one purchase intent.  A mandate is mi, The struct that gets hashed and signed. Field order matches EIP-712., True once a specific merchant and basket have been committed to. (+35 more)

### Community 21 - "InMemoryMandateStore"
Cohesion: 0.15
Nodes (15): MandateAlreadyExists, MandateNotFound, MandateStatusConflict, NonceAlreadyClaimed, Exception, Errors raised by the Mandate Service and its stores.  Note what is *not* here: t, Base class for everything this package raises deliberately., A nonce was reused across two mandates. Breaks one-time consumption. (+7 more)
...
Cohesion: 0.09
Nodes (24): BaseSettings, MonkeyPatch, demo_config(), The Verifier settings the fixtures were generated under., Path, Self, Everything the decision function needs beyond the request itself., Load settings from a TOML file, letting the environment override them. (+16 more)

### Community 24 - "ReviewHold"
Cohesion: 0.11
Nodes (15): BaseModel, datetime, Self, StrEnum, A charge held for a human decision.  CLAUDE.md is explicit that REVIEW needs a h, A paused charge, its evidence, and the moment it stops waiting., ReviewHold, ReviewOutcome (+7 more)

### Community 25 - "MandateNotFound"
Cohesion: 0.19
Nodes (14): BindRequest, build_router(), KillSwitchRequest, MintRequest, APIRouter, BaseModel, HTTP surface for the Mandate Service.  Routers are built by a factory that close, A human approving a REVIEW: name the merchant and the basket. (+6 more)

### Community 26 - "MandateService"
Cohesion: 0.09
Nodes (24): Check, Timestamp, The panic button: stop settlement for everyone., Stop settlement for one buyer., Put a Verifier decision into the audit trail.          The Verifier cannot do th, The one place an audit entry is constructed., CheckKind, CheckResult (+16 more)

### Community 27 - "AuditEntry"
Cohesion: 0.12
Nodes (12): Everything that has happened to one mandate, oldest first., AuditEntry, BaseModel, The audit trail.  Append-only, keyed by mandate. Every state change and every ve, One immutable record. Ordered by `occurred_at` within a mandate., Entries for one mandate, oldest first., DynamoAuditLog, Append-only. Sorted by time within a mandate, so reads are already ordered. (+4 more)

### Community 28 - "to_bytes"
Cohesion: 0.50
Nodes (3): build_router(), APIRouter, HTTP surface for the Verifier Service.  One endpoint, and it takes everything it

### Community 29 - ".deadline_for"
Cohesion: 0.20
Nodes (10): timedelta, The earlier of the review window and the mandate's own expiry.          Taking t, Review deadlines.  CLAUDE.md is explicit: REVIEW needs a hold state and a timeou, A human cannot be given longer to approve than the mandate has to live., Naive datetimes would make every expiry comparison a coin flip., test_a_hold_never_outlives_its_mandate(), test_a_hold_starts_pending_and_records_who_resolved_it(), test_a_short_review_window_wins_over_a_long_mandate() (+2 more)

### Community 30 - "._transition"
Cohesion: 0.11
Nodes (11): Commit a mandate to one merchant and one basket, and re-sign it.          This i, Kill one mandate. Revocation is a deterministic FAIL thereafter., Spend a mandate, exactly once.          This is where replay protection actually, AuditEventType, StrEnum, MandateRecord, A mandate as it is stored: the signed struct plus its lifecycle., DynamoMandateStore (+3 more)
...
Cohesion: 0.53
Nodes (5): FastAPI, create_app(), create_demo_app(), Wires the services into one FastAPI app.  Two factories. `create_app` takes serv, The whole rail, offline: a local issuer key and in-memory stores.      The Verif
...
- **Why does `ScenarioBuilder` connect `ScenarioBuilder` to `VerificationContext`, `test_store_contract.py`, `VerifierService`, `MandateRecord`, `FakeKmsClient`, `test_api.py`, `LocalSigner`, `Mandate`, `mandate_digest`, `Currency`, `.from_toml`?**
  _High betweenness centrality (0.184) - this node is a cross-community bridge._
- **Why does `MandateService` connect `test_mandate_service.py` to `create_demo_app`, `ScenarioBuilder`, `KillSwitchStore`, `MandateRecord`, `test_api.py`, `LocalSigner`, `mandate_digest`, `InMemoryMandateStore`, `MandateNotFound`, `MandateService`, `AuditEntry`, `to_bytes`, `._transition`?**
  _High betweenness centrality (0.171) - this node is a cross-community bridge._
- **Why does `Money` connect `ScenarioBuilder` to `VerifierService`, `test_mandate_service.py`, `test_money.py`, `MandateRecord`, `mandate_digest`, `Currency`, `MandateNotFound`, `._transition`?**
